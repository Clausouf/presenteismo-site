...
export default function DashboardPage() {
  ...
  const dashboardData = useMemo(() => {
    ...
    // --- LÓGICA DE CLASSIFICAÇÃO (RECRUTAMENTO VS ANDAMENTO) ---
    ...
    const totalDesligAndamento = andamentoPool.filter(c => c.isDesligado).length;

    // Métricas Recrutamento
    const totalRecrutamento = recrutamentoPool.length;
    const perdasRecrutamento = recrutamentoPool.filter(c => c.isDesligado).length;

    // Rankings
    const getRankings = (status: 'andamento' | 'recrutamento') => {
      const opsUnicas = [...new Set(turmasFiltradas.map(t => t.operacoes?.nome).filter(Boolean))];
      return opsUnicas.map(opNome => {
        const turmasOp = turmasFiltradas.filter(t => t.operacoes?.nome === opNome);
        const nums = turmasOp.map(t => t.numero_turma);
        const colabsOp = colabsAnalise.filter(c => nums.includes(c.numero_turma) && (status === 'recrutamento' ? c.isRecrutamento : !c.isRecrutamento));
        
        if (status === 'recrutamento') {
          return { nome: opNome, valor: colabsOp.length > 0 ? (colabsOp.filter(c => c.isDesligado).length / colabsOp.length) * 100 : 0 };
        } else {
          const diarioOp = diario.filter(d => nums.includes(d.numero_turma));
          const regs = diarioOp.filter(d => d.tipo_registro !== 'Folga').length;
          const faltas = diarioOp.filter(d => ['Falta Injustificada', 'Falta Integração', 'Atestado'].includes(d.tipo_registro)).length;
          return { nome: opNome, valor: regs > 0 ? (faltas / regs) * 100 : 0 };
        }
    ...
    return {
      metricas: {
        ...
        toMensal: andamentoPool.length > 0 ? (totalDesligAndamento / andamentoPool.length) * 100 : 0,
        absMensal: totalReg > 0 ? (totalFaltas / totalReg) * 100 : 0,
        recrTotal: totalRecrutamento,
        recrPerda: totalRecrutamento > 0 ? (perdasRecrutamento / totalRecrutamento) * 100 : 0
      },
      salaStats,
      rankings: {
        andamentoAbs: getRankings('andamento'),
        recrutamentoPerda: getRankings('recrutamento')
      }
    };
  ...
  return (
    <div className="p-4 space-y-4 text-sm">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        <div className="bg-white p-3 rounded shadow border-l-4 border-red-500"><p className="text-[10px] font-bold text-gray-500 uppercase">TO Mensal (Andam.)</p><p className="text-xl font-bold">{dashboardData.metricas.toMensal.toFixed(1)}%</p></div>
        <div className="bg-white p-3 rounded shadow border-l-4 border-yellow-500"><p className="text-[10px] font-bold text-gray-500 uppercase">ABS Mensal (Andam.)</p><p className="text-xl font-bold">{dashboardData.metricas.absMensal.toFixed(1)}%</p></div>
        <div className="bg-purple-50 p-3 rounded shadow border-l-4 border-purple-500"><p className="text-[10px] font-bold text-purple-600 uppercase">Base Recrutamento</p><p className="text-xl font-bold">{dashboardData.metricas.recrTotal}</p></div>
        <div className="bg-purple-50 p-3 rounded shadow border-l-4 border-purple-700"><p className="text-[10px] font-bold text-purple-600 uppercase">Perda Recrutamento</p><p className="text-xl font-bold">{dashboardData.metricas.recrPerda.toFixed(1)}%</p></div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="grid grid-rows-2 gap-4">
            <div className="bg-white p-4 rounded shadow border border-red-100">
                <h2 className="font-bold text-red-600 mb-2">Ranking ABS - Andamento</h2>
                <div className="space-y-1">
                    {dashboardData.rankings.andamentoAbs.map((o, i) => (
                        <div key={i} className="flex justify-between text-xs border-b pb-1"><span>{o.nome}</span><span className="font-bold">{o.valor.toFixed(0)}%</span></div>
                    ))}
                </div>
            </div>
            <div className="bg-white p-4 rounded shadow border border-purple-100">
                <h2 className="font-bold text-purple-600 mb-2">Ranking Recrutamento</h2>
                <div className="space-y-1">
                    {dashboardData.rankings.recrutamentoPerda.map((o, i) => (
                        <div key={i} className="flex justify-between text-xs border-b pb-1"><span>{o.nome}</span><span className="font-bold">{o.valor.toFixed(0)}%</span></div>
                    ))}
                </div>
            </div>
